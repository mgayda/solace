### Future improvements:

- Vector indexing for full text search. Search is limited right now.
- Error handling and schema validation could be generalized/centralized for consistency along with observability.
- Organization could be improved for scalability.
- Front end should be split into separate styled components.
- Test coverage could be added.
- Styling and theme are basic.
